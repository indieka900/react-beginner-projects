    ...
    return (
        <div className="star-rating">
            {[...Array(fullStars)].map((_, index) => (
                <span key={`full_${index}`} className="star full">★</span>
            ))}
            {partialStar > 0 && (
                <span className="star partial" style={{ '--percent': `${partialStar * 100}%` }}>★</span>
            )}
            {[...Array(emptyStars)].map((_, index) => (
                <span key={`empty_${index}`} className="star empty">★</span>
            ))}
